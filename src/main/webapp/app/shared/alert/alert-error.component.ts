...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('donationApp.error', (response: JhiEventWithContent<AlertError>) => {
      const errorResponse = response.content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('donationApp.httpError', (response: JhiEventWithContent<HttpErrorResponse>) => {
      ...
      switch (httpErrorResponse.status) {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName = translateService.instant('donationApp.' + fieldError.objectName + '.' + convertedField);
        ...
        case 500:
          this.addErrorAlert('Internal Server Error', httpErrorResponse.error.message, httpErrorResponse.error.params);
          break;
